...
def json_to_rows(
    ...
    sort_by_line_count: bool = False,
) -> List[dict]:
    if sort_by_line_count:
        print("→ pre-calculating line counts for sorting...")
        enriched_records = []
        for idx, rec in enumerate(records, 1):
            print(f"\r  - inspecting record {idx}/{len(records)}...", end="", flush=True)
            try:
                repo = ensure_repo(rec["repo_url"], cache_dir, rec["before_fix_sha"])
                rel_path = str(
                    pathlib.PurePosixPath(rec["path_to_buggy_file"], rec["buggy_file_name"])
                )
                pre_code = get_file_cached(repo, rec["before_fix_sha"], rel_path)
                enriched_records.append(
                    {"rec": rec, "line_count": len(pre_code.splitlines())}
                )
            except (FileNotFoundError, KeyError) as e:
                print(
                    f"\n! skipping record {rec.get('issue_id')} during sort pre-calculation: {e}"
                )

        print("\n→ sorting records by line count (shortest first)")
        enriched_records.sort(key=lambda x: x["line_count"])
        ordered = [x["rec"] for x in enriched_records]
        print(f"✓ sorted {len(ordered)} records")
    else:
        # Group by commit pair to maximise cache hits
        ordered = sorted(
            records, key=lambda r: (r["repo_url"], r["before_fix_sha"], r["after_fix_sha"])
        )
...
def main():
    ...
    ap.add_argument(
        ...
        help="Sort records by line count (shortest first). Disables default commit-based sorting.",
    )
    args = ap.parse_args()
    ...
    rows = json_to_rows(
        ...
        args.repo_cache,
        args.max_rows,
        sort_by_line_count=args.sort_by_line_count,
    )
