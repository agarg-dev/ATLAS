...
# Maps pretrain_type -> (dim_model, target_dim, num_head, num_layer)
# These values must match the pretraining hyperparameters in training.py.
_PRETRAIN_CONFIG: dict[str, tuple[int, int, int, int]] = {
    "350M": (1024, 1024, 16, 2),
    "2B":   (2560, 1024, 16, 2),
    "6B":   (4096, 1024, 16, 2),
    "16B":  (6144, 1024, 16, 2),
...
def get_model_config(pretrain_type: str) -> dict[str, int]:
    """Return the VoltronTransformerPretrained architecture config for *pretrain_type*.

    All callers (training, evaluation, demo, window scripts) must use this
    function instead of hardcoding architecture values so that a checkpoint
    trained with one script can be loaded by any other without shape mismatches.

    Returns a dict with keys: dim_model, target_dim, num_head, num_layer.
    """
    if pretrain_type not in _PRETRAIN_CONFIG:
        raise ValueError(
            f"Unknown pretrain_type '{pretrain_type}'. "
            f"Valid choices: {list(_PRETRAIN_CONFIG)}"
        )
    dim_model, target_dim, num_head, num_layer = _PRETRAIN_CONFIG[pretrain_type]
    return {
        "dim_model": dim_model,
...
class TokenizeMask:
    ...
    def padding_pretrain(self, input):
        ...
        sample_padding = torch.zeros(
            self.max_token_len - sample_shape, self.dim_model, device=dev
        )
        final_hidden_states = torch.cat([hidden_states, sample_padding], axis=0)
        # Masking
        attention_mask = torch.cat(
            [
                torch.ones(sample_shape, device=dev),
                torch.zeros(self.max_token_len - sample_shape, device=dev),
            ],
            axis=0,
        )
        return (
    ...
    def __init__(self, root_path):
        ...
        if not self.tensor_paths:
            raise FileNotFoundError(
                f"No .pt tensor files found under {root_path!r} "
                "(check that codegen_loading wrote tensors here and data_path is correct)."
            )
        self.max_file = int(self.tensor_paths[-1])
        root = os.path.normpath(root_path)
        for i, file in enumerate(self.tensor_paths):
            self.tensor_paths[i] = os.path.join(root, f"{file}.pt")
...
class VoltronTransformerPretrained(nn.Module):
    ...
    def forward(self, embedding, attention_mask):
        ...
        """
        # Ensure input and layer weights share the same dtype to avoid matmul dtype mismatch
        if embedding.dtype != self.dim_projection.weight.dtype:
            embedding = embedding.to(self.dim_projection.weight.dtype)

        attention_mask = torch.where(
            attention_mask[:, None, None, :] == 1, 0.0, -torch.inf
        )
